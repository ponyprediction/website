...
class Page
{
    ...
    public function compute()
    {
        ...
        $this->loadJavascript(
                $this->root . '/scripts/' . $this->pageTitle . '.js');
        $this->loadJavascript($this->root . '/scripts/constants.js');
        $this->addJavascript(
                'CONSTANTS = new Constants("' . Constants::$ROOT_URL . '");');
        /*
    ...
    public function getHeader()
    {
        ...
        $header = '<header class="horizontal">
		    <img class="cell" src="' .
                 Constants::$ROOT_URL . '/images/logo.png" 
		        alt="logo">
		    <div id="navs" class="vertical extend">
			    <div class="extend"></div>
		        <nav id="main-nav" class="horizontal">
				    <a id="home" href="' . $this->root .
                 '/home">' . $this->getTextFromDatabase('home') . '</a>
				    <a id="home" href="' . $this->root .
                 '/predictions">' . $this->getTextFromDatabase('predictions') . '</a>
			    </nav>
			    <div class="extend"></div>
			    <nav id="nav-options" class="horizontal">
			        <a id="home" href="' . $this->root .
                 '/language">' . $this->getTextFromDatabase('language') . '</a>
				    <a id="home" href="' . $this->root .
                 '/options">' . $this->getTextFromDatabase('options') . '</a>';
        if ($_SESSION['user']->isConnected())
        {
            $header = $header . '
						    <a id="home" href="' . $this->root .
                     '/account">' . $this->getTextFromDatabase('account') . '</a>
						    <a id="home" href="' . $this->root .
                     '/log-out">' . $this->getTextFromDatabase('log-out') . '</a>
					    ';
        }
        if (! $_SESSION['user']->isConnected())
        {
            $header = $header . '
						    <a id="home" href="' . $this->root .
                     '/register">' . $this->getTextFromDatabase('register') . '</a>
						    <a id="home" href="' . $this->root .
                     '/log-in">' . $this->getTextFromDatabase('log-in') . '</a>
					    ';
        }
        $header = $header .
                 '
	        ...
	        <img class="cell" src="' .
                 Constants::$ROOT_URL . '/images/logo-reversed.png" 
		        alt="logo-reversed">
		</header>';
        return $header;
    }
    ...
    public function getFooter()
    {
		...
		<footer class="footer">
			<p>' .
                 $this->getTextFromDatabase('footer') . '</p>
		</footer>
		';
    }
